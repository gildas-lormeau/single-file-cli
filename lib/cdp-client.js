...
const NETWORK_STATES = ["networkIdle", "networkAlmostIdle", "load", "DOMContentLoaded"];
...
async function getTopFrameContextId({ Page, Runtime }, options) {
	const [contextId] = await new Promise(resolve => {
		...
		function onFrameNavigated({ params }) {
			...
			if (!frame.parentId) {
				...
				resolve(Promise.all([
					getExecutionContextId({ Runtime }, frame.id),
					waitForPageReady({ Page }, frame.id, options)
				]));
			}
		}
	});
...
function getExecutionContextId({ Runtime }, frameId) {
	return new Promise(resolve => {
		...
		function executionContextCreated({ params }) {
			...
			if (origin === "://" && name === SINGLE_FILE_WORLD_NAME && auxData && auxData.frameId === frameId) {
	...
	await new Promise(resolve => {
		...
		function onLifecycleEvent({ params }) {
			...
			if (frameId === topFrameId) {
				if (name === (options.browserWaitUntil || INTERACTIVE_TIME_STATE)) {
