...
async function getPageData(options) {
	...
	try {
		...
		const [contextId] = await Promise.all([
			...
			options.browserDebug ? waitForDebuggerReady({ Debugger }, options) : Promise.resolve()
		]);
...
async function getExecutionContextId({ Page, Runtime }, topFrameId, options) {
	...
	try {
		await waitForPageReady({ Page }, topFrameId, options);
		return contextId;
	} finally {
