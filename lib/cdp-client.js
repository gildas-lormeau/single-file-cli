...
async function getPageData(options) {
	...
	let targetInfo;
	try {
		logData(["Loading page", EMPTY_PAGE_URL], { options, debugMessages });
		...
		await waitDelay({ options, debugMessages });
		...
		logData(["Closing page"], { options, debugMessages });
		await closeTarget();
		logData(["Finishing"], { options, debugMessages });
	}
	...
	async function retryWithFallback() {
		...
		logData(["Retrying with waitUntil", browserWaitUntil], { options, debugMessages });
