...
async function getPageData(options) {
	...
	try {
		...
		return finalizePageData(pageData, httpInfo, { consoleMessages, debugMessages, options });
	} catch (error) {
...
async function setupBindings({ Runtime, Page }, contextId, { options, debugMessages }) {
	...
	Runtime.addEventListener("bindingCalled", async ({ params }) => {
		if (params.name === FETCH_FUNCTION_NAME) {
			await handleFetchRequest({ Runtime }, params, contextId, { debugMessages, options });
		}
	});
}
...
function finalizePageData(pageData, httpInfo, { options, consoleMessages, debugMessages }) {
	if (options.consoleMessagesFile) {
