...
async function setupScriptInjection({ Page, Runtime, Target }, { options, debugMessages }) {
	...
	await setupFrameScriptInjection({ Page, Runtime, Target }, scriptSource, { options, debugMessages });
}
...
async function setupFrameScriptInjection({ Page, Runtime, Target }, scriptSource, { options, debugMessages }) {
...
async function setupBindings({ Page, Runtime }, { options, debugMessages, fetchAbortController }) {
	...
	Runtime.addEventListener(BINDING_CALLED_EVENT_TYPE, ignoringErrors(async ({ params, sessionId }) => {
		if (params.name === FETCH_FUNCTION_NAME) {
			await handleFetchRequest({ Runtime }, params, { options, debugMessages, fetchAbortController }, sessionId);
		}
	}, { options, debugMessages }));
}
...
async function handleFetchRequest({ Runtime }, params, { options, debugMessages, fetchAbortController }, sessionId) {
