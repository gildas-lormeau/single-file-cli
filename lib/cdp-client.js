...
async function getPageData(options) {
	...
	try {
		...
		const contextId = await loadPageAndGetContext({ Debugger, Page, Runtime }, { options, debugMessages });
		...
		return finalizePageData(pageData, { options, consoleMessages, debugMessages, httpInfo });
	} catch (error) {
...
async function loadPageAndGetContext({ Debugger, Page, Runtime }, { options, debugMessages }) {
...
function finalizePageData(pageData, { options, consoleMessages, debugMessages, httpInfo }) {
	if (options.consoleMessagesFile) {
