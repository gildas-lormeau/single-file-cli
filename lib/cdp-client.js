...
async function getPageData(options) {
	...
	try {
		...
		const [{ contextId }] = await Promise.all([
			...
			waitForDebuggerReady({ Debugger }, options)
		]);
...
async function navigate({ Page, Runtime }, options) {
	...
	await Page.enable();
	const [topFrameId] = await Promise.all([getTopFrameId({ Page }), Page.navigate({ url: options.url })]);
	const loadTimeoutAbortController = new AbortController();
	const loadTimeoutAbortSignal = loadTimeoutAbortController.signal;
	const loadTimeoutReady = options.browserLoadMaxTime ? waitForLoadTimeout(loadTimeoutAbortSignal, options.browserLoadMaxTime) : Promise.resolve();
	const contextId = await Promise.race([getExecutionContextId({ Page, Runtime }, topFrameId, options), loadTimeoutReady]);
...
async function getExecutionContextId({ Page, Runtime }, frameId, options) {
	...
	function executionContextCreated({ params }) {
		...
		if (contextId === undefined && context.name === SINGLE_FILE_WORLD_NAME && context.auxData && context.auxData.frameId === frameId) {
	...
	return new Promise(resolve => {
		...
		async function onLifecycleEvent({ params }) {
			...
			if (frameId === topFrameId) {
				if (name === (options.browserWaitUntil || NETWORK_IDLE_STATE)) {
...
async function waitForDebuggerReady({ Debugger }, options) {
	let debuggerReady = Promise.resolve();
