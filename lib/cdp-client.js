...
async function getPageData(options) {
	...
	try {
		...
		await setupConsoleLogging({ Console }, pageContext);
		await setupBrowserWindow({ Browser }, targetInfo.id, pageContext);
		await setupSecurity({ Security }, pageContext);
		await setupDeviceEmulation({ Browser, Emulation, Runtime }, pageContext);
		await setupNetworkInterception({ Browser, Emulation, Fetch, Network }, pageContext);
		await setupScriptInjection({ Page }, pageContext);
		const contextId = await getContextId({ Debugger, Page, Runtime }, pageContext);
		const pageDataPromise = setupPageDataCapture({ Runtime }, contextId, pageContext);
		await setupBindings({ Page, Runtime }, contextId, pageContext);
		await waitDelay(pageContext);
		await capturePageData({ Runtime }, contextId, pageContext);
		await disableCdpDomains({ Console, Network, Page, Runtime }, pageContext);
...
async function waitDelay({ options, debugMessages }) {
