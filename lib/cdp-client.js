...
/* global setTimeout, clearTimeout, URL, fetch, AbortController, window, btoa, Headers */
...
async function getPageData(options) {
	...
	try {
		...
		await setupConsoleLogging(Console, consoleMessages, { options, debugMessages });
		await setupBrowserWindow(Browser, targetInfo, { options, debugMessages });
		await setupSecurity(Security, { options, debugMessages });
		await setupDeviceEmulation({ Browser, Runtime, Emulation }, { options, debugMessages });
		const httpInfo = {};
		await setupNetworkInterception({ Fetch, Network, Browser, Emulation }, httpInfo, { options, debugMessages });
		await injectScripts(Page, options);
		const contextId = await loadPageAndGetContext({ Page, Runtime, Debugger }, options, debugMessages);
		const pageDataPromise = setupPageDataCapture(Runtime, contextId, { options, debugMessages });
		await setupBindings({ Runtime, Page, contextId, options, debugMessages });
		...
		await cleanupProtocols({ Runtime, Page, Network, Console }, options);
		const pageData = await pageDataPromise;
		return finalizePageData(pageData, { consoleMessages, debugMessages, httpInfo, options });
	} catch (error) {
		if (shouldRetryWithFallback(error, options)) {
			return await retryWithFallback(error, options, debugMessages, closeTarget);
		}
		attachDebugInfo(error, { consoleMessages, debugMessages, options });
		throw error;
	} finally {
		if (options.debugMessagesFile) {
...
async function cleanupProtocols({ Runtime, Page, Network, Console }, options) {
...
function finalizePageData(pageData, { consoleMessages, debugMessages, httpInfo, options }) {
	if (options.consoleMessagesFile) {
...
function shouldRetryWithFallback(error, options) {
...
async function retryWithFallback(error, options, debugMessages, closeTarget) {
...
function attachDebugInfo(error, { consoleMessages, debugMessages, options }) {
	if (options.consoleMessagesFile) {
...
async function setupConsoleLogging(Console, consoleMessages, { options, debugMessages }) {
	if (options.consoleMessagesFile) {
		if (options.debugMessagesFile) {
...
async function setupBrowserWindow(Browser, targetInfo, { options, debugMessages }) {
...
async function setupDeviceEmulation({ Browser, Runtime, Emulation }, { options, debugMessages }) {
	...
	if (needsDeviceMetrics) {
		await setupDeviceMetrics({ Runtime, Emulation }, { options, debugMessages });
	}
	if (needsUserAgent) {
...
async function setupDeviceMetrics({ Runtime, Emulation }, { options, debugMessages }) {
...
async function setupNetworkInterception({ Fetch, Network, Browser, Emulation }, httpInfo, { options, debugMessages }) {
	...
	setupRequestInterception(Fetch, httpInfo, { options, debugMessages });
	if (options.httpHeaders) {
...
function setupRequestInterception(Fetch, httpInfo, { options, debugMessages }) {
	...
	Fetch.addEventListener("requestPaused", async ({ params }) => {
		...
		captureHttpInfo(params, httpInfo, urlState, { options, debugMessages });
		if (shouldBlockRequest(request.url, options, debugMessages)) {
			try {
...
function captureHttpInfo(params, httpInfo, urlState, { options, debugMessages }) {
...
function shouldBlockRequest(requestUrl, options, debugMessages) {
...
async function injectScripts(Page, options) {
...
async function loadPageAndGetContext({ Page, Runtime, Debugger }, options, debugMessages) {
...
async function waitForPageReadyState(Page, state, { options, debugMessages }) {
	await new Promise((resolve, reject) => {
		...
		const onLifecycleEvent = createLifecycleEventHandler(state, timeoutState, { options, debugMessages, resolve, cleanup });
		const onFrameNavigated = createFrameNavigatedHandler(state, timeoutState, { options, debugMessages, reject, cleanup });
		Page.addEventListener("lifecycleEvent", onLifecycleEvent);
		Page.addEventListener("frameNavigated", onFrameNavigated);
	});
}

function createLifecycleEventHandler(state, timeoutState, { options, debugMessages, resolve, cleanup }) {
	return ({ params }) => {
...
function createFrameNavigatedHandler(state, timeoutState, { options, debugMessages, reject, cleanup }) {
	return ({ params }) => {
		const { frame } = params;
		if (!frame.parentId) {
...
async function setupBindings({ Runtime, Page, contextId, options, debugMessages }) {
	...
	if (options.embedScreenshot && options.compressContent) {
		await setupScreenshotCapture({ Runtime, Page, contextId, options, debugMessages });
	}
	if (options.embedPdf) {
		await setupPdfCapture({ Runtime, Page, contextId, options, debugMessages });
	}
	...
	Runtime.addEventListener("bindingCalled", async ({ params }) => {
		if (params.name === FETCH_FUNCTION_NAME) {
			await handleFetchRequest(params, { Runtime, contextId, debugMessages, options });
		}
	});
}

async function setupScreenshotCapture({ Runtime, Page, contextId, options, debugMessages }) {
...
async function setupPdfCapture({ Runtime, Page, contextId, options, debugMessages }) {
...
async function handleFetchRequest(params, { Runtime, contextId, debugMessages, options }) {
	const { payload } = params;
