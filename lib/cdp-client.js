...
let canCloseBrowser = false;

export { initialize, getPageData, closeBrowser, canCloseBrowser };
...
async function getPageData(options) {
	...
	try {
		...
		if (targetInfo && !options.browserDebug) {
			const [{ targetInfos }] = await Promise.all([Target.getTargets(), CDP.closeTarget(targetInfo.id)]);
			canCloseBrowser = targetInfos.length == 1;
		}
	}
}
