...
async function launchBrowser(options = {}, indexPath = 0) {
	...
	try {
		...
		await closeBrowser();
		if (options.singleProcess) {
			console.log("Warning: the browser exited when using --browser-single-process, retrying without it"); // eslint-disable-line no-console
...
async function waitUntilReady(debugPort) {
	...
	child.status.then(() => browserExited = true);
	const timeoutTime = Date.now() + READY_TIMEOUT;
	while (!browserExited && Date.now() < timeoutTime) {
		try {
	...
	throw new Error(browserExited ? "The browser exited unexpectedly" : "The browser is not responding");
}
